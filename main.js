...
const { getClaims, checkClaims } = require('./CheckMate');
...
const main = async () => {
    ...
    // Claim detection & verification using Google Fact Check
    // let claims = await getClaims(default_transcript, auth.OPEN_API_KEY);
    // let result = await checkClaims(claims, true);
    ...
    let fact_checked_claim = await factCheckGoogleSearch(claim, auth.OPEN_API_KEY);
    console.log(fact_checked_claim);
}
