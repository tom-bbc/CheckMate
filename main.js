...
const { getClaims, checkClaimArray } = require('./CheckMate');
...
const main = async () => {
    ...
    // Claim detection using OpenAI
    // let claims = await getClaims(default_transcript, auth.OPEN_API_KEY);

    // Some example input claims
    let claims = [
        { Claim: "Every week, 300 of our citizens are killed by heroin alone, 90% of which floods across from our southern border." },
        { Claim: "More Americans will die from drugs this year than were killed in the entire Vietnam war." },
        { Claim: "Over the years, thousands of Americans have been brutally killed by those who illegally entered our country." },
        ...
        { Claim: "The NHS went through the best part of two years where the NHS couldn't conduct all the treatments it normally would." },
        ...
        { Claim: "This year alone, 10,000 people have crossed on boats. That's a record number." }
    ];

    // console.log(claims);

    // Claim verification using Google Fact Check or Google search & OpenAI summary
    const service = "Google search & OpenAI summary";
    let fact_checked_claims = await checkClaimArray(claims, service, auth.OPEN_API_KEY);
}
